...
This project asks whether constraint-based causal discovery can give a clear causal structure for educational intervention data when latent confounding and selection effects are plausible. The empirical case study is the Tennessee Student/Teacher Achievement Ratio (STAR) Project, a randomized study of class size and student achievement. Although STAR is a benchmark for causal analysis, attrition, incomplete background information, school-level heterogeneity, and unobserved socio-economic factors can still affect interpretation. I will develop a Python implementation of Fast Causal Inference (FCI) and FCI+, producing Partial Ancestral Graphs (PAGs) rather than a single assumed causal DAG. The implementation will be benchmarked against existing software and applied to STAR. The outputs will be a tested Python package, benchmark results, and an applied analysis of what FCI+ can and cannot add in this setting.
...
There are two parts to the project. The first is the research question: how far can data-driven causal discovery support conclusions about educational interventions when causal sufficiency cannot be assumed? The second is the programming task: building an implementation that is transparent and robust enough to compare with established tools. I do not claim that FCI+ can replace randomized experimental analysis or estimate treatment effects by itself. Instead, I use it to test whether a defensible causal map can guide downstream modelling.

\section*{Significance}

This project is useful because it connects a theoretically important causal discovery algorithm with a well-known applied dataset. FCI and FCI+ are designed for settings where hidden common causes may exist, which is exactly the kind of uncertainty that often appears in observational and quasi-experimental social-science data. Applying these methods to STAR may show whether the graph learned from the data is consistent with the standard interpretation that smaller classes improve student outcomes, and whether teacher, school, or background variables create additional pathways worth investigating.

A practical motivation is that existing packages often return a graph without making it easy to see why each endpoint was oriented. Many users therefore rely on black-box output without seeing how PAG endpoints, conditional independence tests, separating sets, and orientation rules interact. A self-contained implementation makes these decisions easier to inspect and gives researchers a clearer Python implementation of FCI-style causal discovery.
...
The STAR Project has been widely studied as evidence that smaller early-grade classes can improve student achievement \citep{word1990student,krueger1999experimental}. My aim is not simply to re-estimate this known effect with another standard model. Instead, I use STAR as a test case for latent-variable causal discovery, shifting the focus from one coefficient to a broader causal structure under uncertainty.

\section*{Objectives}

My first objective is to implement a tested Python package for FCI and FCI+, including PAG structures, conditional independence testing, skeleton discovery, Possible-D-SEP refinement, orientation rules, diagnostics, and result export. I will then validate it on synthetic oracle graphs, compare runtime and graph accuracy against reference implementations, and apply the method to STAR. The final objective is to separate conclusions that are reasonably stable from those that remain uncertain because of sample size, measurement, or identifiability limits.

\section*{Methodology}

The implementation starts with core data structures. A PAG representation will store tails, arrowheads, and circles. This matters because FCI output is not a DAG; unresolved circle endpoints are meaningful uncertainty rather than implementation failure. Conditional independence testing will initially use Fisher's Z test for approximately Gaussian variables, with kernel-based or discrete tests considered if time permits. Data validation and caching will be included because repeated tests dominate runtime.
...
The work is planned around the three-month IRP timeline. During the first two weeks, I will complete the implementation plan, literature review, and STAR data familiarisation. Weeks three to five will focus on core FCI implementation, including PAG structures, conditional independence tests, skeleton learning, and collider orientation. Weeks six and seven will extend the implementation toward FCI+ and improve diagnostics, caching, and performance. Weeks eight and nine will run oracle benchmarks and compare reference implementations. Weeks ten and eleven will apply the method to STAR and interpret the learned PAG. The final weeks will be used to write the report, produce visualisations, verify reproducibility, and prepare the software package.
...
The final deliverables will include a Python package implementing FCI and FCI+, a benchmark suite using known oracle graphs, a comparison with existing software, and a STAR case study. The written report will explain the algorithmic design, validation evidence, and applied findings. The aim is to produce a transparent and reproducible account of how FCI+ behaves on both controlled examples and a real educational dataset.
